/**
 * @file  AsyncTCPbuffer.cpp
 * @date  Jan, 2025
 * @author Gerald Guiony
 *
 * AsyncTCPbuffer.cpp for ESP32
 * 
 * Original version from ESPAsyncTCP library (ESP8266) :
...
void AsyncTCPbuffer::_sendBuffer() {
    ...
    while(connected() && (_client->space() > 0) && (_TXbufferRead->available() > 0) && _client->canSend()) {
        ...
        _TXbufferRead->read(out, available);

        // send data
        size_t send = _client->write((const char*) out, available);
        if(send != available) {
            DEBUG_ASYNC_TCP("[A-TCP] write failed send: %d available: %d \n", send, available);
            if ((send >= 0) && (send < available)) {
				// rewrite data not sent to buffer
        		_TXbufferRead->write((const char*) &out[send], available-send);
			}
            if(!connected()) {
...
size_t AsyncTCPbuffer::_handleRxBuffer(uint8_t *buf, size_t len) {
    ...
    if(_RXmode == ATB_RX_MODE_NONE) {
        ...
        if(BufferAvailable > 0) {
            ...
            if(b == NULL){
              return 0; //TODO: What action should this be ?
            }
            // GG
            _RXbuffer->read((char *) b, BufferAvailable);
            r = _cbRX(b, BufferAvailable);
            if ((r >= 0) && (r < BufferAvailable)) {
				// rewrite data not sent to buffer
        		_RXbuffer->write((char *) &b[r], BufferAvailable-r);
			}
        }
